...
public class OVRGLTFAnimatinonNode
{
    ...
    public OVRGLTFAnimatinonNode(JObject jsonData, OVRBinaryChunk binaryChunk, OVRGLTFInputNode inputNodeType,
    ...
    public void AddChannel(JToken channel, JToken samplers)
    {
        int samplerId = channel["sampler"].ToObject<int>();
        var target = channel["target"];
        var extras = channel["extras"];
        int nodeId = target["node"].ToObject<int>();
        OVRGLTFTransformType transformType = GetTransformType(target["path"].ToString());
    ...
    private void ProcessAnimationSampler(JToken samplerNode, int nodeId, OVRGLTFTransformType transformType,
        JToken extras)
    {
        int outputId = samplerNode["output"].ToObject<int>();
        OVRInterpolationType interpolationId = ToOVRInterpolationType(samplerNode["interpolation"].ToString());
        if (interpolationId == OVRInterpolationType.None)
        {
            Debug.LogError("Unsupported interpolation type: " + samplerNode["interpolation"].ToString());
            return;
        }
        ...
        int inputId = samplerNode["input"].ToObject<int>();
        ...
        switch (transformType)
        {
            ...
            case OVRGLTFTransformType.Weights:
                ...
                {
                    m_additiveWeightIndex = extras["additiveWeightIndex"].ToObject<int>();
                }
