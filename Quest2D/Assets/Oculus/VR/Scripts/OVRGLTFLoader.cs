...
public class OVRGLTFLoader
{
    ...
    private int LoadGLTF(bool supportAnimation, bool loadMips)
    {
        ...
        var nodes = m_jsonData["nodes"].ToArray();
        for (int i = 0; i < nodes.Length; i++)
        {
            var jsonNode = m_jsonData["nodes"][i];
			GameObject go = new GameObject(jsonNode["name"].ToString());
			m_Nodes.Add(go);
        }

        // Limit loading to just the first scene in the glTF
        var mainScene = scenes[0];
        var rootNodes = mainScene["nodes"].ToArray();

        // Load all nodes (some models like e.g. laptops use multiple nodes)
        foreach (JToken rootNode in rootNodes)
        {
            int rootNodeId = rootNode.ToObject<int>();
        ...
        return rootNodes[0].ToObject<int>();
    }

	private void ProcessNode(JToken node, int nodeId, bool loadMips)
	{
        ...
        if (childNodes.Count() > 0)
        {
            ...
            {
                int childId = childNodes[i].ToObject<int>();
        ...
        if (node["mesh"] != null)
        {
            var meshId = node["mesh"].ToObject<int>();
            ...
            if (node["skin"] != null)
            {
                ...
                var skinId = node["skin"].ToObject<int>();
        ...
        var translation = node["translation"].ToArray();
        var rotation = node["rotation"].ToArray();
        var scale = node["scale"].ToArray();

        if (translation.Count() > 0)
        {
            Vector3 position = new Vector3(
                translation[0].ToObject<float>() * GLTFToUnitySpace.x,
                translation[1].ToObject<float>() * GLTFToUnitySpace.y,
                translation[2].ToObject<float>() * GLTFToUnitySpace.z);
        ...
        if (rotation.Count() > 0)
        {
            Vector3 rotationAxis = new Vector3(
                rotation[0].ToObject<float>() * GLTFToUnitySpace.x,
                rotation[1].ToObject<float>() * GLTFToUnitySpace.y,
                rotation[2].ToObject<float>() * GLTFToUnitySpace.z);
            rotationAxis *= -1.0f;
            m_Nodes[nodeId].transform.rotation =
                new Quaternion(rotationAxis.x, rotationAxis.y, rotationAxis.z, rotation[3].ToObject<float>());
        }

        if (scale.Count() > 0)
        {
            Vector3 scaleVec = new Vector3(scale[0].ToObject<float>(), scale[1].ToObject<float>(), scale[2].ToObject<float>());
            m_Nodes[nodeId].transform.localScale = scaleVec;
        }
    }

    private OVRMeshData ProcessMesh(JToken meshNode, bool loadMips)
    {
        ...
        for (int i = 0; i < primitives.Count(); i++)
        {
            ...
            var jsonAccessor = m_jsonData["accessors"][jsonAttrbite.ToObject<int>()];

            primitiveVertexCounts[i] = jsonAccessor["Count"].ToObject<int>();
        ...
        {
            matData = ProcessMaterial(jsonMaterial.ToObject<int>());
        ...
        for (int i = 0; i < primitives.Count(); i++)
        {
            ...
            int indicesAccessorId = jsonPrimitive["indices"].ToObject<int>();
    ...
    private OVRMeshAttributes ReadMeshAttributes(JToken jsonAttributes, int totalVertexCount, int vertexOffset)
    {
        ...
        if (jsonAttribute != null)
        {
            ...
            var jsonAccessor = m_jsonData["accessors"][jsonAttribute.ToObject<int>()];
        ...
        if (jsonAttribute != null)
        {
            ...
            var jointAccessor = m_jsonData["accessors"][jointAttribute.ToObject<int>()];
    ...
    private void ProcessSkin(JToken skinNode, SkinnedMeshRenderer renderer)
    {
        Matrix4x4[] inverseBindMatrices = null;
        if (skinNode["inverseBindMatrices"] != null)
        {
            int inverseBindMatricesId = skinNode["inverseBindMatrices"].ToObject<int>();
        ...
        {
            var skeletonRootId = skinNode["skeleton"].ToObject<int>();
        ...
        {
            var joints = skinNode["joints"].ToArray();

            bones = new Transform[joints.Count()];
            for (int i = 0; i < joints.Count(); i++)
            {
                bones[i] = m_Nodes[joints[i].ToObject<int>()].transform;
            }
        }
    ...
    private OVRMaterialData ProcessMaterial(int matId)
    {
        ...
        bool alphaBlendMode = jsonAlphaMode != null && jsonAlphaMode.ToString() == "BLEND";
        ...
        {
            matData.baseColorFactor = new Color(jsonBaseColorFactor[0].ToObject<float>(), jsonBaseColorFactor[1].ToObject<float>(),
                jsonBaseColorFactor[2].ToObject<float>(), jsonBaseColorFactor[3].ToObject<float>());
        }

        var jsonBaseColor = jsonPbrDetails["baseColorTexture"];
        if (jsonBaseColor != null)
        {
            int textureId = jsonBaseColor["index"].ToObject<int>();
        ...
        else
        {
            ...
            {
                int textureId = jsonTextrure["index"].ToObject<int>();
                matData.textureId = textureId;
            }
        }
    ...
    private OVRTextureData ProcessTexture(int textureId)
    {
        ...
        if (jsonExtensions != null)
        {
            ...
            {
                imageSource = baisuExtension["source"].ToObject<int>();
            }
        }
        else
        {
            imageSource = jsonTexture["source"].ToObject<int>();
        }
        ...
        var jsonSourceUri = jsonSource["uri"].ToString();
        ...
        int sampler = jsonTexture["sampler"].ToObject<int>();
        var jsonSampler = m_jsonData["samplers"][sampler];

        int bufferViewId = jsonSource["bufferView"].ToObject<int>();
        ...
        switch (jsonSource["mimeType"].ToString())
        {
            case "image/ktx2":
            ...
            case "image/png":
                ...
                break;
            default:
                Debug.LogWarning($"Unsupported image mimeType '{jsonSource["mimeType"].ToString()}'");
                break;
        }
    ...
    private void ProcessAnimations()
    {
        ...
        foreach (JToken animation in animations.ToArray())
        {
            ...
            var channels = animation["channels"].ToArray();
            foreach (JToken channel in channels)
            {
                int nodeId = channel["target"]["node"].ToObject<int>();
