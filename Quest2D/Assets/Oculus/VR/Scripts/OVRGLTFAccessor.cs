...
public class OVRGLTFAccessor
{
	...
	public OVRGLTFAccessor(JToken node, JObject root, bool bufferViewOnly = false)
	{
		JToken jsonBufferView = node;
		if (!bufferViewOnly)
		{
			additionalOffset = node["byteOffset"].ToObject<int>();
			dataType = ToOVRType(node["type"].ToString());
			componentType = (OVRGLTFComponentType)node["componentType"].ToObject<int>();
			dataCount = node["count"].ToObject<int>();

			int bufferViewId = node["bufferView"].ToObject<int>();
			jsonBufferView = root["bufferViews"][bufferViewId];
		}

		int bufferId = jsonBufferView["buffer"].ToObject<int>();
		byteOffset = jsonBufferView["byteOffset"].ToObject<int>();
		byteLength = jsonBufferView["byteLength"].ToObject<int>();
		byteStride = jsonBufferView["byteStride"].ToObject<int>();

		var jsonBuffer = root["buffers"][bufferId];
		bufferLength = jsonBuffer["byteLength"].ToObject<int>();
	}
